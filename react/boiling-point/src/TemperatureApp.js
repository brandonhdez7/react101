...
class TemperatureApp extends Component{
    ...
    handleCelsiusChange(value){
        console.log("someone changed the celsious input box to " + value)
    ...
    handleFahrenheitChange(value){
        console.log("someone changed the Fahrenheit input box to " + value)
        if(value.target.value.match(/^\d*$/)){
            this.setState({
                temperature: value.target.value,
                scale: "f"
    ...
    render(){
        ...
        if(scale === 'c'){
            ...
            fTemp = (temp * 9 / 5) + 32;
            // no need to convert to caches. its already c.
            cTemp = temp;
        }else if(scale === 'f'){
            fTemp = temp;
            cTemp = (temp - 32) * 5 / 9
